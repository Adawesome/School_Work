...
void* my_alloc(int size) {
	...
	//I manipulate mutate to accommodate for voidSize in order to keep the purity of size.
	int mutate = size;
	while (multiple != 0) {
		mutate++;
	...
	//check to be doubly sure.
	if (myBlock == NULL) {
		// can't do anything, return to end my_alloc
		return 0;
	}
	int checkSplit = (*myBlock).block_size - mutate;
	//if true then can split block
	if (checkSplit > (overheadSize + voidSize)) {
		...
		if (myBlock == free_head)
		{
			...
			(*myBlock).block_size = mutate;//need this because my_free takes a peek at block_size
		}
		else {
			...
			(*myBlock).block_size = splitSize;//only time need to change myBlock block_size
			return tempBlockTest; //we lose what was allocated because we move myBlock so need this temp
		}
	}
	else {
		//Cannot split block
		...
		if (myBlock == free_head) {
			...
			//CANNOT SPLIT, IN MIDDLE OF LIST
			//counter of what is told of me in class, I'm leaving in blockSize = mutate because of how my_free works.
			(*myBlock).block_size = mutate;
			(*prevBlock).next_block = (*myBlock).next_block;
		}
	}
...
void my_free(void *data) {
	...
	//adjust block size becuase we create overhead on my_alloc that isn't represented in block_size, but we have to get rid of that overhead here
	//otherwise we would have 2 overheads... ^^I'm sure most may not have this, but I neeeeed it
...
void testCase3() {
	...
	printf("CURRENT FREE HEAD : ");
	temp = printPointer(free_head);
	printf("free_head should be 16 bytes (my hex's are in bits) ahead of allo4\n");
	printf("______________________________________\n");
}
